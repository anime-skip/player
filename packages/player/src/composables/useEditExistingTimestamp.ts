...
/**
 * Given a timestamp from the edited timestamps list, start editing it. Does
 * nothing if the timestamp is not in the list.
 *
 * See `useCreateTimestamp` to create a new timestamp and start editing it.
 *
 * When passing in an `undefined` timestamp, we still enter edit mode, but we
 * close the edit panel and clear the timestamp being edited.
 */
