...
export interface PlayerOptions {
  /**
   * The streaming service's name, shown above the episode info. Whitespace is
   * trimmed.
   *
   ...
   *
   * Defaults to a `localStorage` based implementation returned by
   * `createLocalPlayerStorage`
   */
  storage?: IPlayerStorage;
  /**
   * A video element, a query selector to one, or a function that returns one of
   * those. Used to get a reference to the video to control it (play, pause,
   * volume, etc).
   *
   * Defaults to `"video"`, a query selector that selects the first video in the
   * DOM.
   */
  video?: ElementOption;
  /**
  ...
  /**
   * Returns information about the current episode and show. You don't need to
   * trim any strings, they will be trimmed automatically. Empty strings will be
   * transformed to `undefined` as well.
   *
   ...
   *
   * Defaults to the shared, heavily rate limited client id:
   * "ZGfO0sMF3eCwLYf8yMSCJjlynwNGRXWE"
   */
  apiClientId?: string;
  /**
   * Return a string representation that should be used when looking up the
   * episode by episode URL on the Anime Skip API.
   *
   * The resulting string should be a valid URL, and contain the minimum
   * information necessary to be unique per episode watched. If you are inside
   * an iframe, it should return the parent page's URL as seen from the search
   * bar.
   *
   * By default, it will simply strip out the query parameters and hash from
   * `location.href`, leaving just the "protocol://host/path".
   */
  ...
  /**
   * Given some bounds, return a URL to the screenshot (usually a base64 encoded
   * data URL).
   */
...
export interface IPlayerStorage {
   ...
   *
   * Setting the value to `null` or `undefined` will set the value to `null`,
   * same as if calling `removeItem`.
   */
