...
export const actions: ActionTree<State, State> & Actions = {
  ...
  async [ActionTypes.LOGIN_MANUAL]({ commit }, { username, password, callback }) {
    try {
      ...
      const loginData = await callApi(commit, window.Api.loginManual, username, password);
  ...
  async [ActionTypes.UPDATE_PREFERENCES](context, preference) {
    ...
    try {
      ...
      try {
        ...
        await callApi(commit, window.Api.updatePreferences, newPreferences);
  ...
  async [ActionTypes.SEARCH_SHOWS]({ commit }, name) {
    try {
      ...
      const results = await callApi(commit, window.Api.searchShows, name);
  ...
  async [ActionTypes.LINK_EPISODE_URL](
    ...
    try {
      await callApi(commit, window.Api.deleteEpisodeUrl, episodeUrl.url);
    } catch (err) {
      // Do nothing
    }
    try {
      await callApi(commit, window.Api.createEpisodeUrl, episodeUrl, episode.id);
  ...
  async [ActionTypes.CREATE_EPISODE_DATA](
    ...
    try {
      ...
      let showId: string;
      if (showData.create) {
        const result = await callApi(commit, window.Api.createShow, {
          name: showData.name,
        });
        showId = result.id;
      } else {
      ...
      if (episodeData.create) {
        const result = await callApi(commit, window.Api.createEpisode, episodeData.data, showId);
        episodeId = result.id;
      } else {
      ...
      if (episodeUrlData.create) {
        try {
          await callApi(commit, window.Api.deleteEpisodeUrl, episodeUrlData.data.url);
        } catch (err) {
          // Do nothing
        }
        episodeUrl = await callApi(
          commit,
          window.Api.createEpisodeUrl,
          episodeUrlData.data,
          episodeId
        );
      } else {
        ...
        try {
          const timestamps = await Promise.all(
            activeTimestamps.map(timestamp =>
              callApi(commit, window.Api.createTimestamp, episodeId, {
  ...
  async [ActionTypes.CREATE_EPISODE_FROM_THIRD_PARTY](
    ...
    try {
      ...
      const showSearchResults = await callApi(commit, window.Api.searchShows, showName);
  ...
  async [ActionTypes.SEARCH_EPISODES]({ commit }, { name, showId }) {
    try {
      ...
      const results = await callApi(commit, window.Api.searchEpisodes, name, showId);
  ...
  async [ActionTypes.FETCH_EPISODE_BY_URL]({ commit }: AugmentedActionContext, url) {
    try {
      ...
      const {
        ...
        ...episodeUrl
      } = await callApi(commit, window.Api.fetchEpisodeByUrl, url);
  ...
  async [ActionTypes.INFER_TEMPLATE]({ commit, state }, payload) {
    try {
      const inferredTemplate = await callApi(
        commit,
        window.Api.findTemplateByDetails,
        payload.episodeId,
        payload.showName,
        payload.season
      );
  ...
  async [ActionTypes.ADD_MISSING_DURATIONS]({ commit, state }, duration) {
    ...
    if (episodeUrl == null || episode == null) {
      ...
      if (newBaseDuration != episode.baseDuration) {
        newEpisode = await callApi(commit, window.Api.updateEpisode, episode.id, {
  ...
  async [ActionTypes.UPDATE_TIMESTAMPS](
    ...
    try {
      for (const toCreateItem of toCreate) {
        await callApi(commit, window.Api.createTimestamp, episode.id, toCreateItem);
      }
      for (const toUpdateItem of toUpdate) {
        await callApi(commit, window.Api.updateTimestamp, toUpdateItem);
      }
      for (const toDeleteItem of toDelete) {
        await callApi(commit, window.Api.deleteTimestamp, toDeleteItem.id);
      }
  ...
  async [ActionTypes.CREATE_TEMPLATE]({ commit, dispatch, state }, payload) {
    ...
    try {
      ...
      const template = await callApi(commit, window.Api.createTemplate, {
  ...
  async [ActionTypes.UPDATE_TEMPLATE]({ commit, dispatch, state }, payload) {
    ...
    try {
      ...
      const template = await callApi(commit, window.Api.updateTemplate, payload.templateId, {
        showId: state.episode.show.id,
        sourceEpisodeId: state.episode.id,
        type: payload.type,
        seasons: payload.seasons,
  ...
  async [ActionTypes.UPDATE_TEMPLATE_TIMESTAMPS]({ commit, state }, payload) {
    ...
    const createPromises = Promise.all(
      toCreate.map(id =>
        callApi(commit, window.Api.addTimestampToTemplate, idToTemplateTimestamp(id))
      )
    );
    const deletePromises = Promise.all(
      toDelete.map(id =>
        callApi(commit, window.Api.removeTimestampFromTemplate, idToTemplateTimestamp(id))
      )
    );
  ...
  async [ActionTypes.DELETE_TEMPLATE]({ commit }, payload) {
    commit(MutationTypes.SET_TEMPLATE_REQUEST_STATE, RequestState.LOADING);
    try {
      await callApi(commit, window.Api.deleteTemplate, payload.templateId);
      commit(MutationTypes.SET_TEMPLATE, undefined);
      commit(MutationTypes.SET_TEMPLATE_REQUEST_STATE, RequestState.SUCCESS);
    } catch (err) {
